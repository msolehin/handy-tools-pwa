...
export function statusOf(
  ...
  if (!candidates.length) return { level: 'ok', days: Infinity, text: 'No trigger set' };
...
```ts
  ...
  garage_fleet: {
    ...
    async write(q, uid, blob) {
      // Children cascade off garage_vehicles, so this delete also clears logs, services,
      // reminders and documents. That is correct — they cannot outlive their vehicle — but it
      // means the other two keys must be re-pushed after a fleet write. The client always
      // writes all three on any structural change; see saveAll() in pages/garage/index.tsx.
      await q('delete from garage_vehicles where user_id = $1', [uid]);
