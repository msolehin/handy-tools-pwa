...
const GroceryBudget: React.FC = () => {
  ...
  const clearChecked = () => {
    if (window.confirm(tr('Buang semua item bertanda dari senarai?', 'Remove every ticked item from the list?'))) {
      setItems(items.filter(item => !item.checked));
    }
  };

  const clearAll = () => {
    if (window.confirm(tr('Buang SEMUA item dan mula senarai baru?', 'Remove EVERY item and start a new list?'))) {
      setItems([]);
    }
  };
  ...
  const diffStr = baseUnitA === 'pcs' ? t('1 unit', '1 unit') : `100${baseUnitA}`;
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex items-center space-x-3 mb-2">
        ...
        <div>
          <h2 className="text-2xl font-bold">{activeTab === 'budget' ? 'Grocery Budget' : t('Banding Harga Seunit', 'Unit Price Comparison')}</h2>
          <p className="text-sm text-muted">{activeTab === 'budget' ? t('Rekod item dan jumlah semasa', 'Track items and your running total') : t('Cari tawaran terbaik secara automatik', 'Find the better deal automatically')}</p>
        </div>
      </div>
      ...
      <div className="flex p-1 bg-black/20 rounded-xl mb-6">
        ...
        >
          {t('Senarai Beli-belah', 'Shopping List')}
        </button>
        <button
        ...
        >
          {t('Banding Harga', 'Compare Prices')}
        </button>
      </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="glass-panel p-5 relative overflow-hidden group">
            ...
            <div className="flex flex-col space-y-4 relative z-10">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-sm text-muted mb-1">{t('Anggaran Jumlah', 'Estimated Total')}</p>
                  <h3 className={`text-4xl font-black font-mono ${isOverBudget ? 'text-red-400' : 'text-green-400'}`}>
                    RM{estimatedTotal.toFixed(2)}
                  </h3>
                </div>
                <div className="text-right">
                  <p className="text-sm text-muted mb-1">{t('Dalam Troli', 'In the Trolley')}</p>
              ...
              <div className="flex items-center space-x-3 bg-black/20 p-3 rounded-xl border border-text/5">
                <div className="flex-1">
                  <p className="text-xs text-muted mb-1">{t('Had Bajet (Pilihan)', 'Budget Limit (Optional)')}</p>
                ...
                {budget > 0 && (
                  <div className="text-right flex-1 border-l border-text/10 pl-3">
                    <p className="text-xs text-muted mb-1">{t('Baki', 'Left')}</p>
          ...
          <form onSubmit={addItem} className="glass-panel p-4 flex items-center space-x-2">
            <div className="flex-1 flex flex-col space-y-2">
              <input 
                ...
                placeholder={t('Nama item (cth. Susu)', 'Item name (e.g. Milk)')}
              ...
              <div className="flex space-x-2">
                ...
                <div className="relative w-24">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted text-xs">{t('Kty', 'Qty')}</span>
                  <input 
                    type="number" 
                    min="1"
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center px-1">
              <h3 className="text-sm font-bold text-muted uppercase tracking-wider">{t('Senarai Beli-belah', 'Shopping List')}</h3>
              {items.length > 0 && (
                <div className="flex space-x-3">
                  <button onClick={clearChecked} className="text-xs text-muted hover:text-text transition-colors">
                    {t('Kosongkan Yang Bertanda', 'Clear Ticked')}
                  </button>
            ...
            {items.length === 0 ? (
              <div className="glass-panel p-8 text-center text-muted">
                ...
                <p>{t('Senarai beli-belah anda kosong.', 'Your shopping list is empty.')}</p>
                <p className="text-sm mt-1">{t('Tambah item di atas untuk mula!', 'Add an item above to start!')}</p>
              </div>
            ) : (
        ...
        <div className="space-y-6 animate-fade-in">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold">
                    {winner === 'TIE' ? t('Kedua-duanya sama nilai.', 'They are the same value.') : t(`Produk ${winner} lebih murah!`, `Product ${winner} is cheaper!`)}
                  </h3>
                  {winner !== 'TIE' && (
                    <p className="text-sm text-muted">
                      {t('Anda jimat', 'You save')} <span className="font-bold text-green-400 font-mono">{savingsPercent.toFixed(1)}%</span> {t(`dengan pilih Produk ${winner}.`, `by choosing Product ${winner}.`)}
                    </p>
                  )}
                </div>
              </div>
          ...
          {baseUnitA !== baseUnitB && sizeA > 0 && sizeB > 0 && (
            <div className="glass-panel p-4 flex items-start space-x-3 bg-red-500/10 border-red-500/20">
              ...
              <p className="text-sm text-muted">
                {t(`Tidak boleh banding ${itemAUnit} dengan ${itemBUnit}. Sila pilih unit jenis sama (cth. isipadu atau berat).`, `${itemAUnit} cannot be compared with ${itemBUnit}. Pick units of the same kind (volume or weight).`)}
              </p>
            </div>
          )}
          ...
          <div className={`glass-panel p-5 space-y-4 ${winner === 'A' ? 'border border-green-500/30' : ''}`}>
            <h3 className="font-bold text-lg">{t('Produk A', 'Product A')}</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-muted mb-1">{t('Harga', 'Price')}</label>
              ...
              <div className="flex space-x-2">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-muted mb-1">{t('Saiz', 'Size')}</label>
                  <input 
                    type="number" 
                    min="0" step="any"
                    value={itemASize}
            ...
            {priceA > 0 && sizeA > 0 && (
              <div className="bg-black/20 p-3 rounded-lg text-center text-sm text-muted border border-text/5">
                <span className="font-mono">RM {unitPriceA.toFixed(2)}</span> {t('setiap', 'per')} {diffStr}
              </div>
            )}
          </div>
          ...
          <div className={`glass-panel p-5 space-y-4 ${winner === 'B' ? 'border border-green-500/30' : ''}`}>
            <h3 className="font-bold text-lg">{t('Produk B', 'Product B')}</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-muted mb-1">{t('Harga', 'Price')}</label>
                <div className="relative">
            ...
            {priceB > 0 && sizeB > 0 && (
              <div className="bg-black/20 p-3 rounded-lg text-center text-sm text-muted border border-text/5">
                <span className="font-mono">RM {unitPriceB.toFixed(2)}</span> {t('setiap', 'per')} {diffStr}
              </div>
            )}
          </div>
