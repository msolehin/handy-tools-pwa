  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pb-24 animate-fade-in">
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted">{t('Disegerak dari Expense Manager', 'Synced from Expense Manager')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="glass-panel p-4 border-l-4 border-l-indigo-500 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1">{t('Belum Bayar', 'Unpaid')}</p>
        ...
        <div className="glass-panel p-4 border-l-4 border-l-emerald-500 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-1">{t('Sudah Bayar', 'Paid')}</p>
        ...
        <div className="glass-panel p-4 border-l-4 border-l-purple-500 relative overflow-hidden group col-span-2">
          ...
          <p className="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-1">{t('Anggaran Setahun (Semua Aktif)', 'Yearly Estimate (All Active)')}</p>
      ...
      >
        <div>
          <p className="font-bold text-sm text-text/90">{t('Urus dalam Expense Manager', 'Manage in Expense Manager')}</p>
          <p className="text-[10px] text-muted">{t('Tambah atau bayar komitmen di sana.', 'Add or pay commitments there.')}</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        {displayedPaid.length > 0 && (
          <div className="space-y-3">
            <h3 className="font-bold text-xs uppercase text-emerald-400 tracking-wider px-1">{t('Sudah Bayar Bulan Ini', 'Paid This Month')}</h3>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center text-xs text-muted mt-2 space-x-3">
                    <span className="flex items-center">
                      ...
                      <span className="font-mono">RM {sub.price.toFixed(2)}</span> {t('/ bln', '/ mo')}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-3">
          <h3 className="font-bold text-xs uppercase text-indigo-400 tracking-wider px-1">{t('Belum Bayar Bulan Ini', 'Unpaid This Month')}</h3>
          {displayedPending.length === 0 ? (
            <div className="text-center p-8 bg-text/5 rounded-2xl border border-text/10 border-dashed">
              <p className="text-muted text-sm">{t('Semua komitmen dalam kategori ini sudah dibayar bulan ini!', 'Every commitment in this category is paid for this month!')}</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center text-xs text-muted mt-2 space-x-3">
                      ...
                      <span className={`flex items-center font-medium ${isOverdue ? 'text-rose-500' : isUrgent ? 'text-rose-400' : 'text-indigo-300'}`}>
                        ...
                        {t('Bayar', 'Due')}: {sub.renewalDate.toLocaleDateString(locale(), { day: 'numeric', month: 'short' })}
                        {isOverdue ? t(` (${Math.abs(daysLeft)}h lewat)`, ` (${Math.abs(daysLeft)}d late)`) : daysLeft === 0 ? t(' (Hari ini)', ' (Today)') : daysLeft === 1 ? t(' (Esok)', ' (Tomorrow)') : t(` (${daysLeft}h)`, ` (${daysLeft}d)`)}
                      </span>
                    </div>
                  </div>
                </div>
