...
const AccountPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    renderSignInButton(buttonRef.current, (ok, message) => {
      if (!ok) setError(message ?? 'Sign-in failed');
    });
  }, [user, online]);
      ...
      <div className="flex items-center gap-3 p-3 rounded-2xl bg-surface border border-text/5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted line-clamp-1 flex items-center gap-1">
            {online
              ? <><ShieldCheck size={12} className="text-emerald-500" /> Saved to your account</>
              : <><WifiOff size={12} /> Offline — changes save when you reconnect</>}
          </p>
        </div>
        <button
          onClick={async () => {
  ...
  return (
    <div className="p-4 rounded-2xl bg-surface border border-text/5 text-center">
      <p className="font-semibold text-text">Guest mode</p>
      <p className="text-xs text-muted mt-1 mb-3">
        Your records stay in this tab only. Sign in to keep them and reach them from any device.
      </p>
      {!authConfigured
        ? <p className="text-xs text-muted">Sign-in isn't set up on this server yet.</p>
        : !online
          ? <p className="text-xs text-muted flex items-center justify-center gap-1"><WifiOff size={12} /> Connect to the internet to sign in.</p>
